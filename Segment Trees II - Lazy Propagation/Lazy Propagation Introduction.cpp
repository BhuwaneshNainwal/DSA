...
void update_range_lazy(int *seg , int ind , int l , int r , int incre , int ss , int se)
{
	if(lazy[ind] != 0)
	{
		seg[ind] += lazy[ind];
	...
	//Otherwise
	int mid = (ss + se) / 2;
	update_range_lazy(seg , 2 * ind , l , r , incre , ss , mid);
	update_range_lazy(seg , 2 * ind + 1 , l , r , incre , mid + 1 , se);
	seg[ind] = min(seg[2 * ind] , seg[2 * ind + 1]);
...
void solve()
{
	...
	update_range_lazy(arr , seg , 1 , 3 , 5 , 10 , 0 , n - 1);
