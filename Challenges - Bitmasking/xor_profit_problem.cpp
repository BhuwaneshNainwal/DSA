...
void solve()
{
	...
	for(int i = 62 ; i >= 0 ; i--)
	{
		...
		if(!check)
		{
			...
			if(arr1[i] == 0 && (curr1 + p) >= orga && (curr1 + p) <= orgb)
			{
				curr1 += p;
				check = true;
			}
			else if(arr1[i] == 0 && (curr2 + p) >= orga && (curr2 + p) <= orgb)
			{
				curr2 += p;
				check = true;
			}
			else if(arr1[i] == 1 && (curr1 - p) >= orga && (curr1 - p) <= orgb)
			{
				curr1 -= p;
				check = true;
			}
			else if(arr1[i] == 1 && (curr2 - p) >= orga && (curr2 - p) <= orgb)
			{
				curr2 -= p;
				check = true;
			}
