...
/*	
	1> It is the function having values - -1 , 0 and 1 only.
	
	2> It is very useful for implementing inclusion exclusion principle.
	
	3> mu(n) returns 0 if n has some primes having square powers(or greater) , - 1 if n
	has square free odd number of distinct primes and 1 if it has even number of distinct 
	square free primes.
*/
...
void call_mobius()
{
	...
	for(int i = 1 ; i < N ; i++)
	{
		//It means it's square free number
		if(a[i] == i)
		{
			if(primes_count[i] % 2 == 0)
				mobius[i] = 1;
			else
				mobius[i] = -1;
		}
