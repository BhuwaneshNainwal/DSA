...
/*
	1> phi(n) represents numbers from 1 to n which are coprimes
	with n.
	
	2> phi(p) = p - 1 , where p is prime	
	
	3> phi(a * b) = phi(a) * phi(b) * g / phi(g) , where g = gcd(a , b)

	4> phi(p ^ k) = phi(p ^ k) - phi( p ^ (k - 1))
	
	5> Let n = (p1 ^ k1) * (p2 * k2) * (p3 ^ k3) then 
	phi(n) = n * (1 - 1 / p1) * (1 - 1 / p2) * (1 - 1 / p3)
	
	6> Let a , b and c are the divisors of n then phi(n) = phi(a) + phi(b) + phi(c) 
