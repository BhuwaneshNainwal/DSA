...
/*
	...
	Flood fill algorithm is the application of dfs
...
void flood_fill(char mat[][50] , int i , int j , char ch , ch color)
{
	if(i < 0 || j < 0 || i >= R || j >= C)
		return;
	 if(mat[i][j] != ch)
	 ...
	 for(int k = 0 ; k < 4 ; k++)
	 	flood_fill(mat , i + dx[k] , y + dy[k] , ch , color);
}
...
void solve()
{
	...
	//Only a single connected component in the graph would be painted with the particular color
	int i , j;
	char ch , color;
	cin >> i >> j >> ch >> color;
	flood_fill(mat , i , j , ch , color);
}
