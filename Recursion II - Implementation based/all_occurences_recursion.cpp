...
int occurences(int *arr , int i , int target , int *output , int j , int n)
{
	//returning count of all occurences
	if(i == n)
		return j;
	if(arr[i] == target)
	{
		output[j] = i;
		return occurences(arr , i + 1 , target , output , j + 1 , n);
	}
	return occurences(arr , i + 1 , target , output , j , n);
...
void solve()
{
	...
	int target;
	cin >> target;
	int m = occurences(arr , 0 , target , output , 0 , n);
