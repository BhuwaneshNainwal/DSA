...
bool check(string s1 , string s2)
{
	...
	string a2 = s1.substr(mid , len - mid);
	string b1 = s2.substr(0 , mid);
	string b2 = s2.substr(mid , len - mid);
	if((check(a1 , b1) && check(a2 , b2)) || (check(a1 , b2) && check(a2 , b1)))
		return true;
	return false;
}
...
void solve()
{
	...
	// int mid = s1.length() / 2;
	// if((s1.substr(0 , mid) == s2.substr(0 , mid) && s1.substr(mid , s1.length() - mid) == s2.substr(mid , s2.length() - mid)) || (s1.substr(0 , mid) == s2.substr(mid , s2.length() - mid) && s1.substr(mid , s1.length() - mid) == s2.substr(0 , mid)))
	// 	cout << "YES" << "\n";
	// else
	// 	cout << "NO" << "\n";
}
