...
int overlap(int arr[] , int l , int r , int mid)
{
	int left = 0 , right = 0;
	int lmx = LLONG_MIN;
	for(int i = mid ; i >= l ; i--)
...
int maximum_sum(int arr[] , int l , int r , int n)
{
	if(l == r)
		return arr[l];
	int mid = (l + r) / 2;
	int left = maximum_sum(arr , l , mid , n);
	int right = maximum_sum(arr , mid + 1 , r , n);
	int between = overlap(arr , l , r , mid);
	return max3(left , right , between) ;
}	
