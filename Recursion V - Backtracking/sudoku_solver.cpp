...
bool is_safe(int matrix[][100] , int i , int j , int number , int n)
{
	for(int x = 0 ; x < n ; x++)
	{
...
void sudoko(int matrix[][100] , int i , int j , int n)
{



	if(j == n)
		return sudoko(matrix , i + 1 , j , n);
	if(matrix[i][j] != 0)
		return sudoko(matrix , i , j + 1 , n);

	for(int number = 1 ; number <= n ; number++)
	{
		if(is_safe(matrix , i , j , number , n))
		{
			matrix[i][j] = number;
			bool couldwesolve = sudoko(matrix , i , j + 1 , n);
			if(couldwesolve)
				return true; 
