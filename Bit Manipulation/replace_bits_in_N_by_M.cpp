...
int clear_range_of_bits_inbetween(int n, int i, int j)
{
    int ones = ~0;
    int a = ones << (j + 1);
    // Since 2 ^ n - 1
...
//Replace N by M at positions i to j in N
int replace(int n , int m , int i)
{
	int ans = (n | (m << i));
	return ans;
}
...
void solve()
{
	int n , m , i , j;
	cin >> n >> m >> i >> j;
	int ans = clear_range_of_bits_inbetween(n , i , j);
	cout << replace(ans , m , i);
