...
void solve()
{
   	...
   	int simulation_time, num_of_nodes, num_of_streets, num_of_cars, bonus_points;
    cin >> simulation_time >> num_of_nodes >> num_of_streets >> num_of_cars >> bonus_points;

    vector<info> vc;
    map<map<int , int> , int> time;
    map<map<int , int> , string> street;
    map<string , pair<int , int>> corresponding_path;
	    
    for(int i = 0 ; i < num_of_streets ; i++)
    {
        int out_edge , in_edge , travel_time;
        string name;
        cin >> out_edge >> in_edge >> name >> travel_time;
		street[out_edge][in_edge] = name;
		time[out_edge][in_edge] = travel_time;
		corresponding_path[name] = {out_edge , in_edge};        
    }
