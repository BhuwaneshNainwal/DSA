...
int solve(int n , int m)
{
	...
	if(!f)
	{
		...
		for(int i = 1 ; i <= 3 ; i++)
		{
				first = ((first % mod) + ((ansn[1][i] % mod) * (F1[i][1] % mod)) % mod) % mod;
		}
	}
	...
	for(int i = 1 ; i <= 3 ; i++)
	{
		second = ((second % mod) + ((ansm[1][i] % mod) * (F1[i][1] % mod)) % mod) % mod;
	}

	int final_answer = (second  - first + mod) % mod;
	return final_answer;
}
