...
int r;
vector<vector<int>> arr(100000 + 10);


vector<vector<int>> seg(4 * 100000 + 10);


vci multiply(vci &a , vci &b)
{
	vci ans(4);
	ans[0] = ((a[0] * b[0]) % r + (a[1] * b[2]) % r) % r;
	ans[1] = ((a[0] * b[1]) % r + (a[1] * b[3]) % r) % r;
	ans[2] = ((a[2] * b[0]) % r + (a[3] * b[2]) % r) % r;
	ans[3] = ((a[2] * b[1]) % r + (a[3] * b[3]) % r) % r;
	return ans;
}
...
void build(int ind, int s , int e)
{
	...
	seg[ind] = multiply(seg[2 * ind] , seg[2 * ind + 1]);
}
...
/*
	O(log n)
*/
vci range_query_point(int ind , int qs , int qe , int ss , int se)
{
	//Completely overlap
	if(ss >= qs && se <= qe)
		return seg[ind];
	...
	//Partial overlap
	int mid = (ss + se) / 2;
	vci left = range_query_point(2 * ind , qs , qe , ss , mid);
	vci right = range_query_point(2 * ind + 1 , qs , qe , mid + 1 , se);
...
void solve()
{
	...
	while(q--)
	{
		...
		qs-- , qe--;
		vci ans(4);
		ans = range_query_point(1 , qs , qe , 0 , n - 1);
		for(int i = 0 ; i < 4 ; i++)
		{
			if(i == 2)
				cout << "\n";
