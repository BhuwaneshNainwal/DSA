...
*/
int n;
int fast_modulo_exponentiation(int x , int n , int m)
{
	int ans = 1;
	while(n)
	{
...
void range_update(int *seg , int ind , int l , int r , int incre , int ss , int se)
{
	...
	if(lazy[ind] != -1)
	{
		if(lazy[ind] == 0)
			...
			seg[ind] = (fast_modulo_exponentiation(2 , (se - ss + 1) , mod) - 1 + mod) % mod;
		if(ss != se)
		{
			lazy[2 * ind] = lazy[ind];
	...
	seg[ind] = ((seg[2 * ind] * fast_modulo_exponentiation(2 , se - mid, mod) % mod) + seg[2 * ind + 1] % mod) % mod;
...
int range_query_lazy(int *seg , int ind , int qs , int qe , int ss , int se)
{
	...
	return ((left * fast_modulo_exponentiation(2 , min(qe , se) - mid , mod) % mod) + right % mod) % mod;
}
...
void solve()
{
	...
	int *seg = new int[4 * n + 100];
	while(q--)
	{
		int type , qs , qe;
		cin >> type >> qs >> qe;
		if(type == 0 || type == 1)
			range_update(seg , 1 , qs , qe , type , 0 , n - 1);
		else
			cout << range_query_lazy(seg , 1 , qs , qe , 0 , n - 1)<< "\n";
	}
	// cout << seg[5];
}
