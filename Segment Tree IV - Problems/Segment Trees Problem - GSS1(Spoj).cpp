...
void build(int ind, int s , int e)
{
	...
	if(s == e)
	{
		seg[ind].presum = arr[s];
		seg[ind].sufsum = arr[s];
		seg[ind].sum = arr[s];
		seg[ind].mxsum = arr[s];
		return;
	}
	...
	treenode left = seg[2 * ind];
	treenode right = seg[2 * ind + 1];

	seg[ind].presum = max(left.presum , left.sum + right.presum);
	seg[ind].sufsum = max(right.sufsum , right.sum + left.sufsum);
	seg[ind].sum = right.sum + left.sum;
	seg[ind].mxsum = max(left.sufsum + right.presum , max(left.mxsum , right.mxsum));
	
}


treenode range_query_point(int ind , int qs , int qe , int ss , int se)
{
	if(ss > se || qe < ss ||qs > se)
		return ((treenode){INT_MIN , INT_MIN , 0 , INT_MIN});
	//Completely overlap
	if(ss >= qs && se <= qe)
		return seg[ind];
	...
	//Partial overlap
	int mid = (ss + se) / 2;
	treenode left = range_query_point(2 * ind , qs , qe , ss , mid);
	treenode right = range_query_point(2 * ind + 1 , qs , qe , mid + 1 , se);

	treenode temp;
	temp.presum = max(left.presum , left.sum + right.presum);
	temp.sufsum = max(right.sufsum , right.sum + left.sufsum);
	temp.sum = right.sum + left.sum;
	temp.mxsum = max(left.sufsum + right.presum , max(left.mxsum , right.mxsum));
	return temp;
}
...
void solve()
{
	...
	while(q--)
	{
		...
		cin >> l >> r;
		l-- , r--;
		treenode t = range_query_point(1 , l , r , 0 , n - 1);
		cout << t.mxsum << "\n";
	}
