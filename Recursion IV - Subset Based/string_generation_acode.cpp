...
vcs ans;
void string_generation(vci &input , string output , int i)
{
	if(i == input.size())
	{
		// cout << output << "\n";
	...
	string_generation(input , output + mp[input[i]] , i + 1);
	if(i + 1 < input.size() && input[i] * 10 + input[i + 1] <= 25)
		string_generation(input , output + mp[input[i] * 10 + input[i + 1]] , i + 2);
...
void solve()
{
	...
	for(auto c : in)
		input.pb(c - '0');
	string output = "";
	string_generation(input , output , 0);
	cout << "[";
