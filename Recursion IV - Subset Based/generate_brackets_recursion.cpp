...
// vcc included(1000);
// set<string> st;
// void generate(string str , string output, int check , int j)
// {
// 	if(check == -1)
// 		return;
// 	if(check == 0 && output.length() == str.length() && st.count(output) == 0)
// 	{
// 		cout << output << "\n";
// 		st.insert(output);
// 		return;
...
// 			temp = -1;
// 		included[i] = 1;
// 		generate(str , output + str[i] , check + temp , i + 1);
// 		included[i] = 0;
// 	}
// }
...
void generate(string output , int open , int close , int n)
{
	// cout << n << " ";
	...
	if(open > close)
		generate(output + ')' , open , close + 1 , n);
	if(open < n)
		generate(output + '(' , open + 1 , close , n);
