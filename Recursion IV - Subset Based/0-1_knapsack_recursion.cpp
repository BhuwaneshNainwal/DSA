...
/*
	Intution is that either we can include the element or we can exclude 
	that element
*/
int maximise(int *prices , int *weight  , int n , int capacity)
{
	if(n == 0 ||  capacity == 0)
		return 0;
	int ans = 0;
	...
	if(weight[n - 1] <= capacity)
		include = prices[n - 1] + maximise(prices, weight , n - 1 , capacity - weight[n - 1]);
	exclude = maximise(prices , weight , n - 1 , capacity);
...
void solve()
{
	...
	int capacity = 7;
	int n = 4;
	cout << maximise(prices , weight , n , capacity);
