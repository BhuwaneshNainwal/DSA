...
string mp[10] = { "" , "" , "ABC" , "DEF" , "GHI" , "JKL" , "KMNO" , "PQRS" , "TUV" , "WXYZ"};

void generate(string input , string output , int i)
{
	if(i == input.length())
	{
		cout << output << "\n";
	...
	for(int k = 0 ; k < mp[digit].length() ; k++)
		generate(input , output + mp[digit][k] , i + 1);
}
