...
void update_range(int *arr , int *seg , int ind , int l , int r , int incre , int ss , int se)
{
	if(r < ss || l > se)
		return;
	...
	//Otherwise
	int mid = (ss + se) / 2;
	update_range(arr , seg , 2 * ind , l , r , incre , ss , mid);
	update_range(arr , seg , 2 * ind + 1 , l , r , incre , mid + 1 , se);
	seg[ind] = min(seg[2 * ind] , seg[2 * ind + 1]);
}

int query(int *arr , int *seg , int ind , int qs , int qe , int ss , int se)
{
	//Completely overlap
	if(ss >= qs && se <= qe)
		return seg[ind];
	...
	//Partial overlap
	int mid = (ss + se) / 2;
	int left = query(arr , seg , 2 * ind , qs , qe , ss , mid);
	int right = query(arr , seg , 2 * ind + 1 , qs , qe , mid + 1 , se);
...
void solve()
{
	...
	update_range(arr , seg , 1 , 3 , 5 , 10 , 0 , n - 1);
