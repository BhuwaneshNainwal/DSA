...
vcs total;
void print(string input , string output , int i)
{
	...
	string temp = table[input[i] - '0'];
	for(int j = 0 ; j < temp.length() ; j++)
	{
		print(input , output + table[input[i] - '0'][j] , i + 1);
	}
...
void solve()
{
	...
	for(int i = 0 ; i < total.size() ; i++)
	{
		for(int j = 0 ; j < 11 ; j++)
		{
			if(included[j] == 1)
				continue;
			int ind = searchIn[j].find(total[i]);
			if(ind >=0 && ind < searchIn[j].length())
			{
				included[j] = 1;
				cout << searchIn[j] << "\n";
			}
