...
mpsi mp;
bool check(char mat[][10] , int i , int j , int m)
{
	if(i < 0 || j < 0 || j >= m || i >= m)
		return false;
	return true;
...
void find(char mat[][10] , int i , int j , string output , int m)
{
	// output.pb(mat[i][j]);
	// mp[output] = 1;
	// if(check(mat , i + 1 , j , m))
	// {

	// 	find(mat , i + 1 , j , output + mat[i + 1][j] , m);
	// }

	// if(check(mat , i + 1 , j + 1 , m))
	// {

	// 	find(mat , i + 1 , j + 1 , output + mat[i + 1][j + 1] , m);
	// }
	...
	// if(check(mat , i  , j - 1 , m))
	// {

	// 	find(mat , i , j - 1, output + mat[i][j - 1] , m);
	// }
