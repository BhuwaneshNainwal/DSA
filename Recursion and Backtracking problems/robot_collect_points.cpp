...
bool check(int board[][100] , int i , int j , int b , int t , int cnt)
{
	if(i < 0 || j < 0 || j > 4)
	{
		if(i < 0 || j < 0 || j > 4)	
...
void maximize(int board[][100] , int i , int j , int b , int t , int cnt , int h)
{
	if(i < 0 || j < 0 || j > 4)
	{
		mx = max(mx, cnt);
	...
	if(check(board , i - 1 , j , b , t , cnt))
	{


		if(board[i - 1][j] == 1)
			maximize(board , i - 1 , j , b , t , cnt + 1 , h);
		else if(board[i - 1][j] == -1 && (i - 1 > b || i - 1 < t))
			maximize(board , i - 1 , j , b , t , cnt - 1 , h);
		else
			maximize(board , i - 1 , j , b , t , cnt , h);
	}
	
	if(check(board , i - 1 , j - 1 , b , t, cnt))
	{

		if(board[i - 1][j - 1] == 1)
			maximize(board , i - 1 , j - 1 , b , t , cnt + 1 , h);
		else if(board[i - 1][j - 1] == -1 && (i - 1 > b || i - 1 < t))
			maximize(board , i - 1 , j - 1, b , t , cnt - 1 , h);
		else
			maximize(board , i - 1 , j - 1, b , t , cnt , h);
	}
...
void solve()
{
	...
	for(int i = h - 1 ; i >= 4 ; i--)
	{
		maximize(board , h , 2 , i , i - 4 , cnt , h);
		mx = max(cnt , mx);
		cnt = 0;
	}
	cout << mx;
