...
bool check(char board[][100] , int i , int j , int n)
{
	if(i < 0 || i >= n || j < 0 || j >= n || board[i][j] == 'k'|| (i - 2 >= 0 && j - 1 >= 0 && board[i - 2][j - 1] == 'k') || (i - 2 >= 0 && j + 1 < n && board[i - 2][j + 1] == 'k') || (i - 1 >= 0 && j + 2 < n && board[i - 1][j + 2] == 'k') || (i - 1 >= 0 && j - 2 >= 0 && board[i - 1][j - 2] == 'k') || (i + 1 < n && j + 2 < n && board[i + 1][j + 2] == 'k') || (i + 1 < n && j - 2 >= 0 && board[i + 1][j - 2] == 'k') || (i + 2 < n && j - 1 >= 0 && board[i + 2][j - 1] == 'k') || (i + 2 < n && j + 1 < n && board[i + 2][j + 1] == 'k'))
		return false;
	return true;
}

void ways(char board[][100] , int i , int j , int cnt , int n , vcpi moves)
{
	// cout << cnt << " ";
	if(cnt == n)
	{
	...
	for(int k = j ; k < n  ; k++)
	{
		...
		if(check(board , i , k , n))
		{
			board[i][k] = 'k';
			moves.pb({i , k});
			ways(board , i , k + 1 , cnt + 1 , n , moves);
			moves.pop_back();
			board[i][k] = '0';
			// ways(board , i , j + 1 , cnt ,  n);
		}	
		
	}
	ways(board , i + 1 , 0 , cnt , n , moves);
