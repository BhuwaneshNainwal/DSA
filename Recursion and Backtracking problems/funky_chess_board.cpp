...
bool check(char board[][11] , int i , int j , int n)
{
	if(i < 0 || i >= n || j < 0 || j >= n || board[i][j] == '0' || board[i][j] == 'k')
		return false;
	return true;
}

void moves(char board[][11] , int i , int j , int cnt , int n)
{
	mx = max(cnt , mx);

	if(check(board , i - 2 , j - 1 , n))
	{
		board[i - 2][j - 1] = 'k';
		moves(board , i - 2 , j - 1 , cnt + 1 , n);
		board[i - 2][j - 1] = '1';
	}
	...
	if(check(board , i - 1 , j + 2 , n))
	{
		board[i - 1][j + 2] = 'k';
		moves(board , i - 1 , j + 2 , cnt + 1 , n);
		board[i - 1][j + 2] = '1';
	}
