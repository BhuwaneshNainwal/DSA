...
/* Binary exponentiation , O(log n) time and O(1) space */
...
/*
	product of divisors of a number N is N ^ (x / 2) , where x is number of divisors of a number N.
...
void solve()
{
	...
	int mm = binpow(2 , mod - 2);
	mm = binpow(mm , n);
		
	int ans = 1;
	for(auto val : arr)
	{
		ans = (ans * ((total  * val) % mod + 2) % mod) % mod;
	}
	ans = (ans * mm) % mod;
	cout << ans;
}
